...
fn vld_server_error_field_errors_all() {
    ...
    assert_eq!(msgs[0], "too short");
    assert_eq!(msgs[1], "must start with uppercase");
}
...
fn validate_value_ok() {
    ...
    assert!(vld_leptos::validate_value::<UserSchema>(&json).is_ok());
}

#[test]
fn validate_value_err() {
    let json = serde_json::json!({ "name": "A", "email": "a@b.com" });
    let err = vld_leptos::validate_value::<UserSchema>(&json).unwrap_err();
    assert!(err.has_field_error(".name"));
}
...
fn check_field_valid() {
    let err = check_field(&"Alice".to_string(), &vld::string().min(2));
    assert!(err.is_none());
}

#[test]
fn check_field_invalid() {
    let err = check_field(&"A".to_string(), &vld::string().min(2));
    assert!(err.is_some());
}

#[test]
fn check_field_number_valid() {
    let err = check_field(&25, &vld::number().int().min(0).max(150));
    assert!(err.is_none());
}

#[test]
fn check_field_number_invalid() {
    let err = check_field(&-5, &vld::number().int().min(0));
    assert!(err.is_some());
}
...
fn validate_args_macro_valid() {
    ...
    let result = vld_leptos::validate_args! {
...
fn error_roundtrip_via_json_string() {
    ...
    // Server side: error → string (e.g. for ServerFnError::new())
    let error_string = err.to_string();

    // Client side: parse the error back
