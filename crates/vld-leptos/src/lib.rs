...
//!
//! Shared validation for Leptos server functions and WASM clients.
...
///
/// vld_leptos::validate::<UserInput, _>(&Args { name, email })?;
/// ```
...
///
/// Designed for reactive client-side validation in Leptos components.
/// Returns `None` if valid, `Some(message)` if invalid.
///
/// # Example
///
/// ```
/// let error = vld_leptos::check_field(&"A".to_string(), &vld::string().min(2));
/// assert!(error.is_some());
///
/// let error = vld_leptos::check_field(&"Alice".to_string(), &vld::string().min(2));
...
///
/// ```
/// let errors = vld_leptos::check_field_all(&"".to_string(), &vld::string().min(2).email());
/// assert!(errors.len() >= 1);
/// ```
...
/// let errors = vld_leptos::check_all_fields::<UserSchema, _>(&data);
...
/// let err = Memo::new(move |_| vld_leptos::check_field(&name.get(), &shared::name_schema()));
/// ```
#[macro_export]
