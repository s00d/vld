//! # vld-dioxus basic example
//!
//! Demonstrates server-side and client-side validation patterns.
//! This example runs as a regular binary (no Dioxus runtime needed).
//!
//! ```sh
...
fn main() {
    println!("=== vld-dioxus validation patterns ===\n");
    ...
    println!("--- Pattern 1: validate_args! macro ---");
    {
        ...
        match vld_dioxus::validate_args! {
            name  => name_schema(),
            email => email_schema(),
            age   => age_schema(),
        } {
    ...
    println!("\n--- Pattern 2: Schema-based validation ---");
    {
        ...
        match vld_dioxus::validate::<CreateUserSchema, _>(&args) {
    ...
    println!("\n--- Pattern 3: check_field (client-side) ---");
    {
        let values = ["Alice", "A", ""];
        for v in &values {
            let err = vld_dioxus::check_field(&v.to_string(), &name_schema());
            match err {
    ...
    println!("\n--- Pattern 4: check_all_fields (whole form) ---");
    {
        ...
        let errors = vld_dioxus::check_all_fields::<CreateUserSchema, _>(&args);
        if errors.is_empty() {
    ...
    println!("\n--- Pattern 5: Error roundtrip ---");
    {
        ...
        let recovered = vld_dioxus::VldServerError::from_json(&json_str).unwrap();
        println!(
