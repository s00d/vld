...
#[doc(hidden)]
pub fn __generate_zod_file_named(schemas: &[(&str, Value)]) -> String {
    let all_schemas = collect_all_schemas(schemas);
    let ordered_schemas = sort_named_schemas_for_codegen(&all_schemas);
    ...
    for (name, schema) in &ordered_schemas {
...
#[doc(hidden)]
pub fn __generate_valibot_file_named(schemas: &[(&str, Value)]) -> String {
    let all_schemas = collect_all_schemas(schemas);
    let ordered_schemas = sort_named_schemas_for_codegen(&all_schemas);
    ...
    for (name, schema) in &ordered_schemas {
        let valibot = convert_schema_valibot(schema);
        lines.push(format!("export const {}Schema = {};", name, valibot));
        lines.push(format!(
...
fn collect_all_schemas(schemas: &[(&str, Value)]) -> Vec<(String, Value)> {
    let mut out = Vec::new();
    ...
    // Auto-collect inline named schemas from common containers.
    let snapshot = out.clone();
    for (_name, schema) in &snapshot {
        collect_inline_named_schemas(schema, &mut out, &mut seen);
    }

    out
}

fn collect_inline_named_schemas(
    schema: &Value,
    out: &mut Vec<(String, Value)>,
    seen: &mut HashSet<String>,
) {
    let mut add_map = |map: &serde_json::Map<String, Value>| {
        for (name, child_schema) in map {
            if seen.insert(name.clone()) {
                out.push((name.clone(), child_schema.clone()));
                collect_inline_named_schemas(child_schema, out, seen);
            }
        }
    };

    if let Some(map) = schema.get("$defs").and_then(|v| v.as_object()) {
        add_map(map);
    }
    if let Some(map) = schema.get("definitions").and_then(|v| v.as_object()) {
        add_map(map);
    }
...
fn sort_named_schemas_for_codegen(schemas: &[(String, Value)]) -> Vec<(String, Value)> {
    use std::collections::{HashMap, VecDeque};

    let mut name_to_idx = HashMap::new();
    for (idx, (name, _)) in schemas.iter().enumerate() {
        name_to_idx.insert(name.clone(), idx);
    }

    let mut indegree = vec![0usize; schemas.len()];
    let mut outgoing: Vec<Vec<usize>> = vec![Vec::new(); schemas.len()];

    for (idx, (_name, schema)) in schemas.iter().enumerate() {
        let refs = collect_ref_paths(schema);
        let mut seen_deps = HashSet::new();
        for ref_path in refs {
            if let Some(dep_name) = extract_ref_name(&ref_path) {
                if let Some(&dep_idx) = name_to_idx.get(dep_name) {
                    if dep_idx != idx && seen_deps.insert(dep_idx) {
                        indegree[idx] += 1;
    ...
    if order.len() != schemas.len() {
        // Cycles: preserve original order for remaining nodes.
        let mut used = vec![false; schemas.len()];
        for &idx in &order {
            used[idx] = true;
        }
        for idx in 0..schemas.len() {
            if !used[idx] {
                order.push(idx);
            }
        }
