...
fn dt_utc_date(dt: OffsetDateTime) -> String {
    dt.date().to_string()
}

fn dt_utc_hm(dt: OffsetDateTime) -> (u8, u8) {
    (dt.hour(), dt.minute())
}
...
fn datetime_parse_rfc3339() {
    ...
    assert_eq!(dt_utc_date(dt), "2024-06-15");
}
...
fn datetime_naive_timezone_offset_changes_interpretation() {
    ...
    assert_eq!(dt_utc_hm(a), (12, 0));
    assert_eq!(dt_utc_hm(b), (9, 0));
}
...
vld::schema! {
    ...
    pub struct Event {
        ...
        pub date: Date => vld::date().min("2020-01-01"),
    }
}
...
fn date_with_default() {
    let fallback = Date::from_calendar_date(2000, Month::January, 1).unwrap();
